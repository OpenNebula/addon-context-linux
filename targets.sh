case "${TARGET}" in
    ...
    'el6_ec2')
        ...
        DEPENDS=${DEPENDS:-util-linux-ng bash curl bind-utils cloud-utils-growpart dracut-modules-growroot parted ruby rubygem-json sudo shadow-utils}
        PROVIDES=${PROVIDES:-}
        REPLACES=${REPLACES:-cloud-init}
    ...
    'el7')
        ...
        DEPENDS=${DEPENDS:-util-linux bash curl bind-utils cloud-utils-growpart parted ruby rubygem-json sudo shadow-utils open-vm-tools qemu-guest-agent openssh-server}
        PROVIDES=${PROVIDES:-}
        REPLACES=${REPLACES:-cloud-init}
        CONFLICTS=${CONFLICTS:-${REPLACES} one-context-ec2}
        POSTIN=${POSTINST:-pkg/postinstall}
    ...
    'suse')
        ...
        TYPE=${TYPE:-rpm}
        TAGS=${TAGS:-rpm systemd one}
        DEPENDS=${DEPENDS:-util-linux bash curl bind-utils growpart parted parted ruby sudo shadow open-vm-tools qemu-guest-agent openssh} # rubygem-json}
        PROVIDES=${PROVIDES:-}
    ...
    'suse_ec2')
        ...
        TYPE=${TYPE:-rpm}
        TAGS=${TAGS:-rpm systemd ec2}
        DEPENDS=${DEPENDS:-util-linux bash curl bind-utils growpart parted ruby sudo shadow} # rubygem-json}
        PROVIDES=${PROVIDES:-}
        REPLACES=${REPLACES:-cloud-init cloud-init-config-suse}
        CONFLICTS=${CONFLICTS:-${REPLACES} one-context}
        POSTIN=${POSTINST:-pkg/postinstall.ec2 pkg/postinstall}
    ...
    'deb')
        ...
        DEPENDS=${DEPENDS:-util-linux bash curl bind9-host cloud-utils parted ruby python ifupdown acpid sudo passwd open-vm-tools qemu-guest-agent dbus openssh-server}
        PROVIDES=${PROVIDES:-}
        REPLACES=${REPLACES:-cloud-init}
        CONFLICTS=${CONFLICTS:-${REPLACES} one-context-ec2}
        POSTIN=${POSTINST:-pkg/postinstall}
    ...
    'deb_ec2')
        ...
        DEPENDS=${DEPENDS:-util-linux bash curl bind9-host cloud-utils parted ruby python ifupdown sudo passwd resolvconf}
        PROVIDES=${PROVIDES:-}
        REPLACES=${REPLACES:-cloud-init}
        CONFLICTS=${CONFLICTS:-${REPLACES} one-context}
        POSTIN=${POSTINST:-pkg/postinstall.ec2 pkg/postinstall}
    ...
    'alpine')
        ...
        TYPE=${TYPE:-apk}
        TAGS=${TAGS:-apk one}
        DEPENDS=${DEPENDS:-util-linux bash curl rsync udev iptables sfdisk parted e2fsprogs-extra keepalived quagga sudo shadow ruby ruby-json bind-tools open-vm-tools qemu-guest-agent openssh-server}
        PROVIDES=${PROVIDES:-}
    ...
    'arch')
        ...
        DEPENDS=${DEPENDS:-filesystem util-linux bash curl bind-tools ruby sudo shadow open-vm-tools qemu-guest-agent openssh}
        PROVIDES=${PROVIDES:-}
        REPLACES=${REPLACES:-cloud-init}
        CONFLICTS=${CONFLICTS:-${REPLACES} one-context-ec2}
        POSTIN=${POSTINST:-pkg/postinstall}
